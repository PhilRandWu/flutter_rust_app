use crate::AppState;
use crate::common::auth::{generate_tokens, verify_password, verify_refresh_token};
use crate::common::error::AppError;
use crate::modules::auth::dto::TokenResponse;
use crate::modules::users::dto::{User, UserToken};
use anyhow::Result;
use chrono::DateTime;
...
impl AppState {
    pub async fn get_token(
        ...
        let (access_token, refresh_token, access_claims, refresh_claims) = generate_tokens(user.clone()).await?;

        let user_token = UserToken::new(
            user.user_info.id.unwrap(),
            refresh_claims.jti,
            DateTime::from_timestamp(refresh_claims.exp, 0).ok_or_else(|| AppError::InternalServerError)?,
        );
    ...
    pub async fn refresh_token(&self, refresh_token: &str) -> Result<TokenResponse, AppError> {
        let refresh_claims = verify_refresh_token(refresh_token).await?;
        ...
        let _ = db_token.ok_or_else(|| AppError::Unauthorized("refresh token invalid/expired".into()))?;
        ...
        let (new_access_token, new_refresh_token, new_claims, new_refresh_claims) = generate_tokens(user.clone()).await?;
        ...
        Ok(TokenResponse::new(&new_access_token, &new_refresh_token, new_claims.exp))
